"""
Enhanced training script with improved hyperparameters - Version 2
"""
...
from src.models.transformer_v2 import NextLocationPredictorV2
...
def get_config_v2():
    ...
    config = {
        ...
        # Model architecture - optimized for <500K params
        'model': {
            'd_model': 192,
            'num_heads': 6,
            'num_layers': 4,
        ...
        # Training settings - more aggressive
        'training': {
            'max_epochs': 150,
...
def main():
    ...
    print("="*80)
    print("NEXT-LOCATION PREDICTION - VERSION 2 (ENHANCED)")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    print("\n" + "="*80)
    print("Creating enhanced model (V2)...")
    print("="*80)
    
    model = NextLocationPredictorV2(
    ...
    # Save results
    results_path = os.path.join(config['results_dir'], 'final_results_v2.txt')
    with open(results_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("FINAL TEST RESULTS - VERSION 2\n")
        f.write("="*80 + "\n")
        ...
        f.write(f"{'='*80}\n")
        f.write("- Larger model capacity (192 vs 128 dimensions)\n")
        f.write("- Deeper architecture (4 vs 3 layers)\n")
        f.write("- Gated feature fusion\n")
        f.write("- Learnable positional embeddings\n")
        f.write("- Pre-layer normalization\n")
        f.write("- GELU activations\n")
        f.write("- Better hyperparameters\n")
